			...
			<div class="container">
				...
				<div class="row">
					<div class="col-md-8">
						<div class="row">
						<%
							if(request.getAttribute("mostPopSongFromCatId") != null) {
								Song objSong = (Song)request.getAttribute("mostPopSongFromCatId");
						...
						%>
							...
							<div class="col-md-12">
								<div class="post post-thumb">
									<a class="post-img" href="<%=songUrl %>"><img width="750px" height="450px" src="<%=request.getContextPath()%>/files/<%=objSong.getSongPicture() %>" alt=""></a>
									<div class="post-body">
										<div class="post-meta">
						...
						<%
							if (request.getAttribute("listSongByCatId") != null) {
								...
								ArrayList<Song> listSongByCatId = (ArrayList<Song>) request.getAttribute("listSongByCatId");
								for (Song objSong: listSongByCatId) {
									String catUrl = request.getContextPath() + "/categories/" + StringUtil.makeSlug(objSong.getSongCat().getCatName()) + "-" + objSong.getSongCat().getCatId() + "-" + 1;
									String songUrl = request.getContextPath() + "/details/" + StringUtil.makeSlug(objSong.getSongName()) + "-" + objSong.getSongId();
						%>	
							<!-- post -->
						...
						<%
					  	...
					  	%>
						    ...
						    <ul class="pagination">
						     <li class="page-item <%if(currentPage == 1) out.print("disabled"); %>"><a href="<%=request.getContextPath()%>/categories/<%=StringUtil.makeSlug(objCat.getCatName())%>-<%=objCat.getCatId() %>-<%if(currentPage - 1 < 1) out.print(currentPage); else out.print(currentPage - 1);%>">&laquo;</a></li>
						     <%
						     	String active = "";
						    	for(int i = 1; i <= numberOfPage; i++){
						    		String catUrl = request.getContextPath() + "/categories/" + StringUtil.makeSlug(objCat.getCatName()) + "-" + objCat.getCatId() + "-" + i;
						    		if(currentPage == i){
						    			active = "active";
						    		} else {
						    			active = "";
						     ...
						     %>
						     <li class="page-item <%=active%>"><a href="<%=catUrl%>"><%=i %></a></li>
						     <%	} %>
						    <li class="page-item <%if(currentPage == numberOfPage) out.print("disabled"); %>"><a href="<%=request.getContextPath()%>/categories/<%=StringUtil.makeSlug(objCat.getCatName())%>-<%=objCat.getCatId() %>-<%if(currentPage + 1 > numberOfPage) out.print(numberOfPage); else out.print(currentPage + 1);%>">&raquo;</a></li>
						   </ul>
						  </div>
						  <%}} %>
						</div>
